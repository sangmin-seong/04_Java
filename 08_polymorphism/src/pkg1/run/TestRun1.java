...
//		 System.out.println(p2.getCar());      
		 ...
		 * 
		 *  업 캐스팅 상태(부모 참조 = 자식 객체)에서 참조 변수의 자료형을
		 *  자식 객체 자료형으로 강제 형변환 하는 것 
		 *  
		 *  -  
		 */
		
		Parent p3 = new Child(); // 업 캐스팅
		
		// p3(Parent)를 Child로 강제 형변환하여 
		// 반환된 주소를 c3에 저장
		Child c3 = (Child)p3;
		
		// 아래 방법처럼 필요 시마다 형변환 가능하지만
		...
		Object[] arr = new Object[3];
							// Object 참조 변수 3개 묶음
		// Object 참조 변수  =  Parent / Child / Object
		 ...
		 *   자식 객체를 참조하는 변수에 부모타입을 검사하면 true가 나온다
		 *   ex)  Parent p = new Child(); // 업 캐스팅
		 *   	  
		 *		  p instanceof Child
		 *		  -> p가 참조하는 객체가 Child가 맞는가? true
		 *
		 *		  p instanceof Parent
		 *        -> p가 참조하는 객체가 Parent가 맞는가? true
		 *
		 *		  p instanceof Object
		 *        -> p가 참조하는 객체가 Object가 맞는가? false
		 *        
		 *        => 주의사항을 잘 이용하면 
