...
//		arr[5] = 10;
	...
	public void test2() {
		...
		//InputMismatchException 던져질 가능성이 있는 코드 작성
		try {
	...
	public void test4() {
		...
		// 자식 타입 예외를 먼저 처리(if)
		// -> 부모 타입을 나머지 예외 처리하는(else) 형식으로 사용
		catch(InputMismatchException e) {
			System.out.println("int 범위 정수만 입력해주세요");
		}
		
		catch( RuntimeException e ) {
			// catch의 매개 변수 타입을 부모 타입으로 설정
			// -> 던져지는 모든 자식타입의 예외를 잡아서 처리
			
			// 장점 : 여러 예외처리 구문을 작성하지 않고도 예외처리 가능
			
			// 단점 : 발생하는 예외별 처리 방법을 제시할 수 없다
			System.out.println("뭔지 모를 예외 발생");
		}
		// InputMismatchException에 빨간줄(컴파일 에러)가 발생하는 이유
		// -> 앞에서 부모타입 RunTimeException을 참조변수로 이용해
		//	  자식 타입 예외를 잡아서 처리했기 때문에
		//    Unreachable(도달할 수 없는) 코드(dead code)가 되어버림.......
		
		// 해결 방법 : 부모처리 구문보다 앞에 작성하면 해결!
//		catch(InputMismatchException e) {
//			System.out.println("int 범위 정수만 입력해주세요");
//		}
	}
	 ...
	 *    - 예외 처리구문 미작성 시 컴파일 에러발생 (빨간줄)
	 *    
	 * 2) unchecked Exception
	 *    - 예외 처리 구문 작성여부가 필수가 아닌 선택하는 예외
	 *    - 예외 처리 구문을 작성하지 않아도 컴파일 에러 미발생
